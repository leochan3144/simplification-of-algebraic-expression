    ...
    function generateLevel1Question() {
        ...
        const types = [
            ...
            () => { // 2p × 3p × p × 4
                ...
                const c2 = Math.floor(Math.random() * 3) + 1;
                const c3 = Math.floor(Math.random() * 3) + 1;
                const c4 = Math.floor(Math.random() * 3) + 1;
                const finalCoeff = c1 * c2 * c3 * c4;
                return { question: `${c1}${p} × ${c2}${p} × ${c3}${p} × ${c4}`, answer: `${finalCoeff}${p}^4` };
            },
            () => { // h × 3h × 3k × k
                ...
                const finalHCoeff = c1;
                const finalKCoeff = c2;

                return { question: `${h} × ${c1}${h} × ${c2}${k} × ${k}`, answer: `${finalHCoeff}${h}^2${finalKCoeff}${k}^2` };
            }
        ];
